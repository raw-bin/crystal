...
module Crystal
  ...
  class Program < NonGenericModuleType
    ...
    {% for name in %w(object no_return value number reference void nil bool char int int8 int16 int32 int64 int128
                     ...
                     markable array static_array exception tuple named_tuple proc union enum range regex crystal
                     packed_annotation thread_local_annotation no_inline_annotation
                     always_inline_annotation naked_annotation returns_twice_annotation
                     raises_annotation primitive_annotation call_convention_annotation
                     flags_annotation link_annotation extern_annotation deprecated_annotation 
                     experimental_annotation weak_annotation stack_realign_annotation internal_annotation) %}
      def {{name.id}}
        @{{name.id}}.not_nil!
      end
    {% end %}
